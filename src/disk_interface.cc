...
TimeStamp StatSingleFile(const std::wstring& path, std::string* err) {
  ...
  if (attrs.dwFileAttributes & FILE_ATTRIBUTE_REPARSE_POINT) {
    HANDLE hFile = CreateFileW(path.c_str(), 0, 0, 0, OPEN_EXISTING,
                               FILE_FLAG_BACKUP_SEMANTICS, 0);
    if (hFile == INVALID_HANDLE_VALUE) {
      DWORD win_err = GetLastError();
      if (win_err == ERROR_FILE_NOT_FOUND || win_err == ERROR_PATH_NOT_FOUND)
        return 0;
      *err = "CreateFileW(" + Win32UnicodeToUTF8(path) +
             "): " + GetLastErrorString();
      CloseHandle(hFile);
      return -1;
    }
    ...
    if (GetFinalPathNameByHandleW(hFile, pathBuf, MAX_PATH,
                                  FILE_NAME_NORMALIZED) == 0) {
      DWORD win_err = GetLastError();
      if (win_err == ERROR_FILE_NOT_FOUND || win_err == ERROR_PATH_NOT_FOUND)
        return 0;
      *err = "GetFinalPathNameByHandleW(" + Win32UnicodeToUTF8(path) +
             "): " + GetLastErrorString();
      CloseHandle(hFile);
      return -1;
    }
...
bool StatAllFilesInDir(const std::wstring& dir,
  ...
  do {
    std::wstring lowername = ToLowercase(ffd.cFileName);
    if (lowername == L"..") {
    ...
    if (ffd.dwFileAttributes & FILE_ATTRIBUTE_REPARSE_POINT) {
      ...
      stamps->insert(make_pair(
          lowername, StatSingleFile(dir + L"\\" + ffd.cFileName, err)));
    } else {
      stamps->insert(
...
bool SystemDiskInterface::WriteFile(const string& path, const string& contents,
                                    bool crlf_on_windows) {
#ifdef _WIN32
  FILE* fp = _wfopen(UTF8ToWin32Unicode(path).c_str(),
                     crlf_on_windows ? L"wt" : L"wb");
#else   // !_WIN32
