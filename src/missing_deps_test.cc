...
struct MissingDependencyScannerTest : public testing::Test {
  ...
  void CreateInitialState() {
    ...
    state_.AddOut(header_edge, CanonicalPath("generated_header"), nullptr);
    Edge* compile_edge = state_.AddEdge(&compile_rule_);
    state_.AddOut(compile_edge, CanonicalPath("compiled_object"), nullptr);
  }
  ...
  void CreateGraphDependencyBetween(const char* from, const char* to) {
    ...
    state_.AddIn(from_edge, CanonicalPath(std::string(to)));
  }
...
TEST_F(MissingDependencyScannerTest, MissingDepFixedIndirect) {
  ...
  state_.AddOut(intermediate_edge, CanonicalPath("intermediate"), nullptr);
