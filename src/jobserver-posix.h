...
class PosixJobserverClient : public Jobserver::Client {
 public:
  // Create new instance from a pair of file descriptors.
  // On failure, set |*error| then return null.
  static std::unique_ptr<Jobserver::Client> Create(
    ...
    if (config.mode == Jobserver::Config::kModeFileDescriptors) {
      success = client->InitWithFds(config.read_fd, config.write_fd, error);
    } else if (config.mode == Jobserver::Config::kModePosixFifo) {
      success = client->InitWithFifo(config.path, error);
    } else {
      assert(false && "Unsupported jobserver mode!!");
    }
    if (!success)
 ...
 protected:
  PosixJobserverClient() = default;

  bool InitWithFds(int read_fd, int write_fd, std::string* error) {
...
class PosixJobserverPool : public Jobserver::Pool {
 public:
  static std::unique_ptr<PosixJobserverPool> Create(
    ...
    if (mode == Jobserver::Config::kModeFileDescriptors) {
  ...
  std::string GetEnvMakeFlagsValue() const override {
    ...
    if (!fifo_.empty()) {
      ...
      int ret = snprintf(const_cast<char*>(result.data()), result.size(),
                         " -j --jobserver-auth=fifo:%s", fifo_.c_str());
    ...
    if (read_fd_ >= 0 && write_fd_ >= 0) {
      ...
      int ret = snprintf(const_cast<char*>(result.data()), result.size(),
                         " -j --jobserver-fds=%d,%d --jobserver-auth=%d,%d",
                         read_fd_, write_fd_, read_fd_, write_fd_);
      if (ret < 0 || ret > static_cast<int>(result.size()))
        Fatal("Could not format PosixJobserverPool MAKEFLAGS!");
      result.resize(static_cast<size_t>(ret));
    }
