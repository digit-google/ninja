...
rebuild those targets. Since Ninja 1,12, the `--type` option can be used
to change the set of inputs printed, for example
+ninja -t inputs --type=explicit _targets_+ will only print explicit inputs.
Use `--type=list` to list all possible values and purpose.
