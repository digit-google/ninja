...
Since version x.xx, Ninja understands the GNU jobserver protocol on Posix.

NOTE: This feature does not work on Windows at the moment!

If the `MAKEFLAGS` environment variable is defined, it will assume that
it is running in the context of a higher-level job slot pool, and will
parse its value to interact with it. In this case, Ninja acts as a
client for the coordination protocol, by limiting the number of commands
it dispatches appropriately, and passing the `MAKEFLAGS` variable to its
sub-processes as well.

Otherwise, Ninja can setup its own job slot pool and act as a protocol
server on its own. This is enabled by using parallelism and using the
`--jobserver` option. Alternatively, the `NINJA_JOBSERVER` environment
variable can be set to select a Jobserver implementation mode. Valid
values are:
...
NOTE: The `fifo` mode corresponds to `--jobserver-style=fifo` introduced
      by GNU Make 4.4, and some protocol clients may not support it
      (Ninja does when running as a jobserver client).

NOTE: The Ninja-provided pool implementation is basic and does not
      support broken protocol clients which release more tokens than
      they acquire. If such clients are used in your build, expect
      severe build performance degradation over time.
...
`NINJA_JOBSERVER` can also be defined in the environment to specify
a jobserver mode, in the case where it is not possible to use the
corresponding `--jobserver=MODE` option.

If `MAKEFLAGS` is defined in the environment, Ninja will assume that
it is already running in the context of a GNU jobserver pool of
job slots, and will ignore `--jobserver` and `NINJA_JOBSERVER` values
entirely.
